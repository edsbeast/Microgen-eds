...
class Tpms(Shape):
    ...
    def _compute_offset_to_fit_density(
        ...
        """Compute the offset to fit the required density."""
        if self.density is None:
            raise ValueError(f"density must be between 0 and 1. Given: {self.density}")
    ...
    def density_from_offset(
        ...
        """Return the density corresponding to the given offset."""
        temp_tpms = Tpms(
            surface_function=surface_function,
            offset=offset,
