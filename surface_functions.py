...
def SDFRD(x: np.ndarray,  y: np.ndarray, z: np.ndarray) -> np.ndarray:
    S = 2
    D = 3
    FRD = 4
    return(
        4 * S * (cos(x) + cos(y) + cos(z)) + 
        4 * D * (cos(x)*cos(y)*cos(z)-sin(x)*sin(y)*sin(z)) + 
        FRD * (4 * cos(x) * cos(y) * cos(z) - (cos(2 * x) * cos(2 * y) + cos(2 * y) * cos(2 * z) + cos(2 * z) * cos(2 * x)))
        )

def SSPIWP(x: np.ndarray,  y: np.ndarray, z: np.ndarray) -> np.ndarray:
    S = 2
    SP = 3
    IWP = 4
    return(
